...
export PII_EMBED_DEVICE="${PII_EMBED_DEVICE:-cuda}"
export TORCH_INSTALL_PACKAGE="${TORCH_INSTALL_PACKAGE:-torch}"
export TORCH_EXTRA_INDEX_URL="${TORCH_EXTRA_INDEX_URL:-https://download.pytorch.org/whl/cu118}"
export NVIDIA_VISIBLE_DEVICES="${NVIDIA_VISIBLE_DEVICES:-all}"
export NVIDIA_DRIVER_CAPABILITIES="${NVIDIA_DRIVER_CAPABILITIES:-compute,utility}"

echo "Starting gRPC GPU direct mode with 1 replica, ${GRPC_WORKERS} worker(s), embed_device=${PII_EMBED_DEVICE}"
