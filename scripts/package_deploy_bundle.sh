...
write_sanitized_env() {
  local target_path="$1"
  {
    ...
    echo "PII_LOG_REQUEST_TEXT_LIMIT=240"
    echo "PII_LOG_REQUEST_TEXT_REDACT_SENSITIVE=true"
    ...
    if [[ -f "${PROJECT_ROOT}/.env" ]]; then
      while IFS= read -r line || [[ -n "${line}" ]]; do
        case "${line}" in
          ""|\#*) continue ;;
          PII_IMAGE_REPO=*|PII_IMAGE_TAG=*|PII_RUNTIME_BACKEND=*|PII_EMBED_DEVICE=*|COMPOSE_FILE=*|PII_GRPC_SCALE=*|PII_GRPC_MAX_WORKERS=*|PII_DETECT_PROCESS_WORKERS=*|PII_DETECT_QUEUE_LIMIT=*|PII_SPLIT_MAX_WORKERS=*|PII_CONTEXT_RULE_FIRST_ENABLED=*|PII_CONTEXT_EMBED_NORMALIZE_DIGITS=*|PII_MODEL_PRELOAD_ENABLED=*|PII_STAGE_TIMING_ENABLED=*|PII_LOG_REQUEST_TEXT_ENABLED=*|PII_LOG_REQUEST_TEXT_LIMIT=*|PII_LOG_REQUEST_TEXT_REDACT_SENSITIVE=*|PII_LOG_MAX_FILE_MB=*|PII_LOG_TOTAL_MAX_MB=*|PII_LOG_ROTATE_INTERVAL_SEC=*|PII_DOCKER_LOG_MAX_SIZE=*|PII_DOCKER_LOG_MAX_FILE=*) continue ;;
          *) echo "${line}" ;;
        esac
