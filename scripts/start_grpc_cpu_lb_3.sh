...
echo "Starting gRPC CPU LB mode with 3 replicas, ${GRPC_WORKERS} worker(s)"
...
docker compose -f docker-compose.grpc-cpu.yml --profile grpc up -d --no-build --scale api-grpc=3 api-grpc
