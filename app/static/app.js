...
function formatJson(value) { return escapeHtml(JSON.stringify(value ?? {}, null, 2)); }
  ...
  for (const t of types) {
    ...
    for (const it of arr) {
      ...
      spans.push({
        ...
        ,context_pass: typeof it.context_pass === 'boolean' ? it.context_pass : null
      });
    }
  }
...
function renderTableRows(spans) {
  return spans.map(s => {
    const isValidText =
      ((s.tag === "SN" || s.type === "SN") && typeof s.isValid === "boolean")
...
function renderTypeCounts(data) {
  const keys = ["SN", "FN", "SSN", "DN", "PN", "MN", "BRN", "BN", "AN", "CN", "CPN", "CRN", "IMEI", "MCN", "EML", "VN_CCCD", "VN_MN", "VN_PN", "VN_TIN", "VN_SI", "OTP", "API_KEY", "AUTH_TOKEN", "PASSWORD", "INTERNAL_ACCESS", "PRIVATE_KEY", "CLOUD_CREDENTIAL", "CONNECTION_STRING", "SIGNED_URL", "MFA_SECRET", "RECOVERY_CODE", "SESSION_COOKIE"];
  return keys
...
function toggleGrpcDetectFields() {
  const methodEl = document.getElementById("grpcMethod");
  const detectFieldsEl = document.getElementById("grpcDetectFields");
  if (!methodEl || !detectFieldsEl) return;
  detectFieldsEl.style.display = methodEl.value === "Detect" ? "block" : "none";
}

async function loadGrpcDefaults() {
  const targetEl = document.getElementById("grpcTarget");
  const timeoutEl = document.getElementById("grpcTimeout");
  try {
    const res = await fetch("/grpc/test/defaults");
    if (!res.ok) return;
    const payload = await res.json();
    if (targetEl && !targetEl.value) targetEl.value = payload?.target || "";
    if (timeoutEl && !timeoutEl.value) timeoutEl.value = String(payload?.timeout_sec || "10");
  } catch (_) {
...
async function runGrpcTest() {
  ...
  const methodEl = document.getElementById("grpcMethod");
  const timeoutEl = document.getElementById("grpcTimeout");
  const useTlsEl = document.getElementById("grpcUseTls");
  const textEl = document.getElementById("grpcText");
  const maxEl = document.getElementById("grpcMaxSpans");
  const rulesetEl = document.getElementById("grpcRuleset");
  const outEl = document.getElementById("grpcOut");
  const timingEl = document.getElementById("grpcTiming");

  const method = methodEl?.value || "Health";
  const body = {
    target: (targetEl?.value || "").trim(),
    method,
    use_tls: !!useTlsEl?.checked,
    timeout_sec: Number(timeoutEl?.value || "10"),
    text: textEl?.value || "",
    max_results_per_type: parseInt(maxEl?.value || "200", 10),
    ruleset: (rulesetEl?.value || "").trim() || null,
  };

  if (!body.target) {
    if (outEl) outEl.innerHTML = `<div class="result bad">대상 주소를 입력하세요.</div>`;
    return;
  }

  if (outEl) outEl.innerHTML = `<div class="card">gRPC 호출 중...</div>`;
  if (timingEl) {
    timingEl.style.display = "block";
    timingEl.innerHTML = "요청 중...";
  }

  const t0 = performance.now();
  try {
    const res = await fetch("/grpc/test", {
      method: "POST",
      headers: { "content-type": "application/json" },
    ...
    if (timingEl) {
      timingEl.innerHTML =
        `<b>걸린시간</b> · 요청(서버응답) ${(t1 - t0).toFixed(1)} ms · ` +
        `JSON파싱 ${(t2 - t1).toFixed(1)} ms · 총 ${(t2 - t0).toFixed(1)} ms`;
    }
    if (outEl) {
      const summary = ok
        ? `<div class="result ok"><b>${escapeHtml(method)}</b> 호출 성공 · target=${escapeHtml(payload?.target || body.target)} · server=${escapeHtml(String(payload?.elapsed_ms ?? "-"))} ms</div>`
        : `<div class="result bad"><b>${escapeHtml(method)}</b> 호출 실패 · target=${escapeHtml(payload?.target || body.target)} · server=${escapeHtml(String(payload?.elapsed_ms ?? "-"))} ms<br/>${escapeHtml(payload?.error?.code || payload?.error?.type || "error")} ${escapeHtml(payload?.error?.details || payload?.error?.message || "")}</div>`;
      outEl.innerHTML = `
        ${summary}
        <div class="card">
          <div class="subTitle">응답 JSON</div>
          <pre class="jsonView">${formatJson(payload)}</pre>
        </div>
      `;
    }
  } catch (e) {
    const t1 = performance.now();
    if (timingEl) timingEl.innerHTML = `<b>걸린시간</b> · ${(t1 - t0).toFixed(1)} ms (네트워크 오류)`;
    if (outEl) {
      outEl.innerHTML = `
        <div class="result bad">요청 실패: ${escapeHtml(e?.message || String(e))}</div>
      `;
    }
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("tab-grpc")?.addEventListener("click", () => setActiveTab("grpc"));
  ...
  document.getElementById("btnGrpcRun")?.addEventListener("click", runGrpcTest);
  document.getElementById("grpcMethod")?.addEventListener("change", toggleGrpcDetectFields);
  ...
  const grpcTextArea = document.getElementById("grpcText");
...
이메일 변형: user.name_01@sub.example.com, ADMIN@EXAMPLE.ORG`;
    ...
    if (grpcTextArea && !grpcTextArea.value) grpcTextArea.value = sampleText;
  }
