...
def build_pipeline(bundle: RuleBundle) -> List[Detector]:
    ...
    sensitive_steps = {
        ...
        "private_key": ("PRIVATE_KEY", docs.get("private_key") or {}),
        "cloud_credential": ("CLOUD_CREDENTIAL", docs.get("cloud_credential") or {}),
        "connection_string": ("CONNECTION_STRING", docs.get("connection_string") or {}),
        "signed_url": ("SIGNED_URL", docs.get("signed_url") or {}),
        "mfa_secret": ("MFA_SECRET", docs.get("mfa_secret") or {}),
        "recovery_code": ("RECOVERY_CODE", docs.get("recovery_code") or {}),
        "session_cookie": ("SESSION_COOKIE", docs.get("session_cookie") or {}),
    }
    numeric_separator_doc = docs.get("numeric_separator") or {}
    evasion_recovery_doc = docs.get("evasion_recovery") or {}
    ...
    hs_cn_db: HyperscanDB | None = None
    ...
    cn_verify_regexes: List[Pattern] = []
    ...
    if _env_bool("PII_HS_CN_ENABLED", True):
        try:
            hs_cn_db = _build_hs_db_regex_rule(cn_doc)
            cn_verify_regexes = _build_verify_regexes(cn_doc)
        except HyperscanUnsupportedRule:
            pass
        except Exception as e:
            print(f"[warn] CN Hyperscan disabled due to compile error: {e}")
    ...
    if _env_bool("PII_HS_COMBINED_ENABLED", False):
        try:
            ...
            if hs_cn_db is not None:
                combined_specs.append(("CN", cn_doc, {}))
            if combined_specs:
    ...
    for step in steps:
        ...
        if step == "cn":
            max_len = int(cn_doc.get("max_match_len") or max_len_default)
            hs_db = _select_hs_db("CN", hs_cn_db)
            if hs_db is not None:
                pipeline.append(
                    HSRegexDetector(
                        "CN",
                        hs_db=hs_db,
                        enabled=bool(cn_doc.get("enabled", True)),
                        max_match_len=max_len,
                        verify_regexes=cn_verify_regexes,
                        verify_window_chars=1,
                        supplement_regexes=cn_supplement_regexes,
                    )
                )
            else:
        ...
        if step == "post_mn":
            ...
            pipeline.append(
                MNPostFilter(
                    ...
                    alternate_separator=mn_doc.get("alternate_separator"),
                )
            )
            continue
        ...
        if step == "post_bn":
            ...
            pipeline.append(
                BNPostFilter(
                    ...
                    alternate_separator=bn_doc.get("alternate_separator"),
                )
            )
            continue

        if step == "post_context":
            if not _env_bool("PII_CONTEXT_ENABLED", True):
                continue
        ...
        if step == "numeric_separator":
            pipeline.append(NumericAlternateSeparatorDetector(numeric_separator_doc))
            continue

        if step == "evasion_recovery":
            pipeline.append(EvasionRecoveryDetector(evasion_recovery_doc))
            continue
